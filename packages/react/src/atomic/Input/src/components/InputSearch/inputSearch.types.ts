...
export type InputSearchBaseProps = InputSearchProperties &
  ...
  // and `Input.Search`) stopped being assignable here. The `Omit` itself
  // strips every native key `InputProperties` also declares — not just
  // `prefix`, but `disabled` too — yet `disabled` comes straight back through
  // the `Pick` above, so the composed type gives up exactly one native prop:
  // the RDFa `prefix`.

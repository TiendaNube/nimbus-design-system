 ...
 */
function iconComponentName(filename: string): string {
...
/**
 * Exposes the concrete icon exports in the source map so consumers do not
 * need to list and inspect every asset just to verify that an icon exists.
 *
 * The package's SVGR template appends `Icon` to the generated component name.
 */
function collectIconExports(assetsDir: string): string[] {
  if (!fs.existsSync(assetsDir)) return [];

  return fs
    .readdirSync(assetsDir, { withFileTypes: true })
    .filter((entry) => entry.isFile() && entry.name.endsWith(".svg"))
    .map((entry) => `${iconComponentName(entry.name)}Icon`)
    .sort(compareStrings);
}
  ...
  if (storyIndex) {
    const storiesFile = srcFiles.find(
      (file) =>
        ...
        isConventional(file, normalizedName)
    );
...
function enrichSharedEntries(
  ...
  const shared = Object.fromEntries(
    Object.entries(sharedEntries).map(([key, value]) => [
      key,
      { ...value },
    ])
  );
  ...
  if (icons) {
    const assets = path.posix.join(icons.path, "src/assets");
    const absoluteAssetsPath = path.join(cwd, ...assets.split("/"));
...
export function generateSourceMap(
  ...
  const workspaceByLocation = new Map(
    workspaces.map((workspace) => [
      workspace.location.split(path.sep).join("/"),
      workspace.name,
    ])
  );
